...
{
	char vector2[READLINELEN + 1], *ptr2, *retptr,
		newvector[READLINELEN + 1];
/* char cbuff[MED_BUFFER_SIZE+1],rbuff[MED_BUFFER_SIZE+1],tmpbuff[MED_BUFFER_SIZE+1]; */
	...
	int retval, samerepeat;
	if (debug & 1)
		fprintf(fpstatus, "deal_with_repeat_DPM\n");
	cbuff = malloc(MED_BUFFER_SIZE + 1);
	if (cbuff == 0) {
		fprintf(stderr, "FATAL_ERROR:Could not get memory,cbuff\n");
		fprintf(fperrors,
				"FATAL_ERROR:%s:Could not get memory,cbuff[%s]line%ld\n",
	...
	if (rbuff == 0) {
		...
		fprintf(fperrors,
				"FATAL_ERROR:%s:Could not get memory,rbuff[%s]line%ld\n",
				progstr, infile_orig, readlinenum);
		exit(RET_NOT_ENOUGH_MEMORY);
	}
	...
	if (tmpbuff == 0) {
		...
		fprintf(fperrors,
				"FATAL_ERROR:%s:Could not get memory,tmpbuff[%s]line%ld\n",
				progstr, infile_orig, readlinenum);
		exit(RET_NOT_ENOUGH_MEMORY);
	}
	...
	*repeat_cnts = repeatcnt;
	...
	if (debug & 8)
		fprintf(fpstatus, "FYI:rcb_rpt:%ld rpt_nst:%d ", repeat_size,
				*repeat_nesting);
	for (i = 0; i < *(repeat_nesting) - 1; i++)
		repeat_size *= repeat_count_buffer[i];
	fprintf(fpstatus, " final size:%ld\n", repeat_size);
	if (FORCEDPM == 0 && repeat_size > MAX_LOOP_SIZE) {
		fprintf(fpstatus, "returning loop size too big(2) [%s,%d,%s]\n",
				__FILE__, __LINE__, __FUNCTION__);
		return (RET_LOOP_SIZE_TOO_BIG);
	}
	fprintf(fpstatus, "We are continuing\n");
	if (SCAN && repeatcnt == RET_FOREVER)
		return (RET_REPEAT_FOREVER_SCAN);
	else if (repeatcnt == RET_FOREVER)
		return (RET_REPEAT_FOREVER);	/* cant do repeat forever in DPM */
	else if (repeatcnt == 0) {
/* this could be bad syntax or not really what we were looking for, ie. inside a comment */
	...
	fprintf(fpstatus, "found repeat [%06ld] LEVEL[%d]\n", repeatcnt,
			*repeat_nesting);
	samerepeat = 0;
	if (strstr(vector, "begin") != NULL) {
		fprintf(fpstatus, "   and begin!!\n");
		samerepeat = 1;
		vector2[0] = '\0';		/* no read if found begin in same as repeat line */
	}
	else {
...
/*                                   begin might be on the same line */
	if (NULL == strstr(vector2, "begin") && samerepeat == 0) {	/* not begin */
		fprintf(fpstatus,
				"oops, not a repeat block,just a repeat vector.\n");
		appr_vc += (repeatcnt - 1);
		if (*(repeat_nesting) == 1) {
			ptr = strchr(vector2, '[');
		...
		while (fgets(vector2, READLINELEN - 1, fpin1) != NULL) {
			if (debug & 16)
				fprintf(fpstatus, "V:[%s]\n", vector2);
			readlinenum++;
			vector2[strlen(vector2) - 1] = '\0';
			if (NULL != strstr(vector2, "GCT"))
				ilooprc++;
			if (0 == strstr(vector2, "end")) {	/* not at end */
				if (debug & 16)
					fprintf(fpstatus, "continuing search for end!!!!\n");
				if ((ptr2 = strstr(vector2, "repeat")) == NULL) {	/* not a repeat */
					if (NULL != (ptr2 = strchr(vector2, '['))) {	/* probably a vector */
						expand_vector(newvector, vector2, ptr2, __LINE__,
									  __FILE__);
					...
					repeat_count_buffer[*repeat_nesting - 1] =
						*repeat_cnts;
					if (debug & 8)
						fprintf(fpstatus, "FYI:rcb1:%d %d\n",
								repeat_count_buffer[*repeat_nesting - 1],
								*repeat_nesting);
					rbuff[0] = '\0';
					/* the following is somewhat dangerous as it is a recursive call!!! */
					if (debug & 1)
						fprintf(fpstatus, "pre-deal_with_repeat_DPM5\n");
					retval =
						deal_with_repeat_DPM(fpin1, fpout1, ptr2,
											 repeat_nesting, loop_number,
											 vector2, repeat_cnts, rbuff);
					if (debug & 1)
						fprintf(fpstatus, "post-deal_with_repeat_DPM5\n");
				...
				if (debug & 8)
					fprintf(fpstatus, "FOUND END!!!!\n");
				appr_vc += (ilooprc * repeatcnt);
				if (*repeat_nesting > 1) {	/* if not at top level, we must use retbuff */
					rbuff[0] = '\0';
					chksize =
						repeatcnt * (strlen(cbuff) +
									 strlen
									 ("(*BOL#%ld Level=%d LOOP=%ld of %ld*) \n")
									 + strlen("(*EOL#%ld LOOP%ld*) \n"));
					if (chksize > MED_BUFFER_SIZE) {
						fprintf(stderr,
								"FATAL_ERROR:LOOP expansion[%ld] would exceed buffer size[%d]\n",
								chksize, MED_BUFFER_SIZE);
						fprintf(fperrors,
								"FATAL_ERROR:%s:LOOP expansion would exceed buffer size[%s]line%ld\n",
								progstr, infile_orig, readlinenum);
						exit(1);
					}
	...
	free(tmpbuff);
	if (debug & 1)
		fprintf(fpstatus, "deal_with_repeat_DPM done\n");
	return (RET_GOOD);
}
...
{
	char vector2[READLINELEN + 1], vector_prev[READLINELEN + 1], *ptr2,
		*retptr;
	...
	cbuff[0] = '\0';
	if (debug & 1)
		fprintf(fpstatus, "deal_with_match\n");
	...
	if (0 == strstr(vector2, "begin")) {	/* not begin */
		fprintf(fpstatus,
				"oops, not a match block,just a match vector.\n");
		fprintf(fpout1, "%s\n", vector);
		ptr = strchr(vector2, '[');
		expand_vector(newvector, vector2, ptr, __LINE__, __FILE__);
		fprintf(fpout1, "%s\n", vector2);
		...
		fprintf(fpout1, "$%s_M_%d_%d_%d\n", patnameptr,
				(int) *repeat_nesting, (int) repeatcnt,
				(int) *loop_number);
		...
		while (fgets(vector2, READLINELEN - 1, fpin1) != NULL) {
			...
			if (0 == strstr(vector2, "end")) {	/* not at end */
				...
				if ((ptr2 = strstr(vector2, "repeat")) == 0) {	/* not a repeat */
					...
					fprintf(fpstatus,
							"*** WARNING: found another repeat inside!!!! ***\n");
					fprintf(fpstatus,
							" Microinstructions could be wrong in this case \n");
					/* the following is somewhat dangerous as it is a recursive call!!! */
					if (debug & 1)
						fprintf(fpstatus, "pre-deal_with_repeat_CPM6\n");
					deal_with_repeat_CPM(fpin1, fpout1, ptr2,
										 repeat_nesting, loop_number,
										 vector2, repeat_cnts, cbuff,
										 patnameptr);
					if (debug & 1)
						fprintf(fpstatus, "post-deal_with_repeat_CPM6\n");
				}
			}
			else {				/* found end !!! */
				if (debug & 8)
					fprintf(fpstatus, "FOUND END!!!!\n");
				fprintf(fpstatus,
						" Attempting to add <COND SEQF> back in \n");
				fprintf(fpout1, "<DC1,CJMP .+3>\n");
				fprintf(fpout1, "<MODE(IFAIL OFF,TCI OFF)>\n");
				fprintf(fpout1,
						"%s  (* NSFAPP--this should have strobes*)\n",
						vector_prev);
...
//         else fprintf(fpout1,"%s\n<STOP FAIL>(* removing this STOP FAIL and adding STROBES will only work on VX250/VX500 not on EX or VX4, APS 02/27/06 rev15+ pre_proc *)\n",vector_prev);
	...
	if (debug & 1)
		fprintf(fpstatus, "deal_with_repeat_match done\n");
	return (RET_GOOD);
}
...
int deal_with_repeat_CPM(FILE * fpin1, FILE * fpout1, char *ptr,
						 ...
						 char *patnameptr)
{
	char vector2[READLINELEN + 1], newvector[READLINELEN + 1], *ptr2,
		*retptr;
	char cbuff[MED_BUFFER_SIZE + 1];
	long repeatcnt = 0;
	...
	if (debug & 1)
		fprintf(fpstatus, "deal_with_repeat_CPM\n");
	if ((*repeat_nesting) == 0)
		retbuff[0] = '\0';		/* only wipe when at top level */
	(*repeat_nesting)++;
	if ((*repeat_nesting) > 4) {
		fprintf(stderr,
				"NON_FATAL_ERROR:Not enough CPM counters available for nesting this deep.\n");
		fprintf(fperrors,
				"NON_FATAL_ERROR:%s:Not enough CPM counters available for nesting this deep.[%s]line%ld\n",
				progstr, infile_orig, readlinenum);
		fprintf(stderr,
				"(NON_FATAL_ERROR):Attempting translation anyway, you will need to figure this one out!!!\n");
		fprintf(fperrors,
				"(NON_FATAL_ERROR):%s:Attempting translation anyway, you will need to figure this one out!!!\n",
				progstr);
	}
	fflush(fpout1);
	retptr = get_count_orig(ptr, sizeof("repeat"), &repeatcnt, vector);
	*repeat_cnts = repeatcnt;
	if (repeatcnt == RET_FOREVER)
		repeat_forever = 1;
	else if (repeatcnt == 0) {
	...
	if (0 == strstr(vector2, "begin")) {	/* not begin */
		if (repeat_forever) {
			...
			fprintf(fpout1, "$%s_RF_%d_%d_%d\n", patnameptr,
					(int) *repeat_nesting, 0, tmpholdloop);
			ptr = strchr(vector2, '[');
			expand_vector(newvector, vector2, ptr, __LINE__, __FILE__);
			fprintf(fpout1, "%s\n", newvector);
		...
		if (repeat_forever) {
			...
			if (repeatcnt > 65535) {
				fprintf(fpout1, "<LC%d %d>\n<LC%d %d>\n<COND NZC%d>\n",
						4 - (int) *repeat_nesting, 10 - 1,
						5 - (int) *repeat_nesting,
						(int) repeatcnt / 10 - 1,
						5 - (int) *repeat_nesting);
				if (repeatcnt - 10 * (int) (repeatcnt / 10))
					fprintf(fpout1,
							"(* check me! %d=%d*%d missing loops: %d *)\n",
							(int) repeatcnt, 10, (int) repeatcnt / 10,
							(int) repeatcnt - 10 * (int) (repeatcnt / 10));
				countlines += 4;
				...
				fprintf(fpout1, "<LC%d %d>\n<COND NZC%d>\n",
						5 - (int) *repeat_nesting, (int) repeatcnt - 1,
						5 - (int) *repeat_nesting);
				countlines += 2;
				doubleloop &= ~(1 << (4 - (int) *repeat_nesting));
			}
			...
			fprintf(fpout1, "$%s_R_%d_%d_%d\n", patnameptr,
					(int) *repeat_nesting, (int) repeatcnt, tmpholdloop);
			countlines++;
		}
		while (fgets(vector2, READLINELEN - 1, fpin1) != NULL) {
			readlinenum++;
			...
			if (NULL == strstr(vector2, "end")) {	/* not at end */
				...
				if ((ptr2 = strstr(vector2, "repeat")) == 0) {	/* not a repeat */
					if (NULL != (ptr = strchr(vector2, '['))) {
						expand_vector(newvector, vector2, ptr, __LINE__,
									  __FILE__);
						strcpy(vector2, newvector);
					}
					fprintf(fpout1, "%s\n", vector2);
					...
					/* the following is somewhat dangerous as it is a recursive call!!! */
					if (debug & 1)
						fprintf(fpstatus, "pre-deal_with_repeat_CPM7\n");
					deal_with_repeat_CPM(fpin1, fpout1, ptr2,
										 repeat_nesting, loop_number,
										 vector2, repeat_cnts, cbuff,
										 patnameptr);
					if (debug & 1)
						fprintf(fpstatus, "post-deal_with_repeat_CPM7\n");
				}
			}
			else {				/* found end !!! */
				if (debug & 8)
				...
				if (repeat_forever) {
					...
					if ((doubleloop & (1 << (4 - (int) *repeat_nesting)))
						...
						fprintf(fpout1, "<DC%d,CJMP %s_R_%d_%d_%d-2>\n",
								4 - (int) *repeat_nesting, patnameptr,
								(int) *repeat_nesting, (int) repeatcnt,
								tmpholdloop);
						countlines++;
						fprintf(fpout1,
								"(* check me! count might be off slightly*)\n");
						countlines++;
						doubleloop &= ~(1 << (4 - (int) *repeat_nesting));
					}
					else {
	...
	(*repeat_nesting)--;
	if (debug & 1)
		fprintf(fpstatus, "deal_with_repeat_CPM done\n");
	return (RET_GOOD);
}
