(*****************************************************************************)
(***           TMX370E16D17  Program from Standard Mega1 Shell             ***)
(***                                                                       ***)
(***                    File Name:  device.p                               ***)
(***                                                                       ***)
(*****************************************************************************)
(***              Source Code Control System Header (SCCS)                 ***)
(***                                                                       ***)
(***               SHELL MODULE       :  device.shell                      ***)
(***               VERSION NUMBER     :  2.1                               ***)
(***               LAST MODIFICATION  :  4/27/90  15:18:19                 ***)
(***               PREVIOUS DELTA     :  4/27/90  15:18:12                 ***)
(*** SCCS: //node_12a11/local_user/petemc/shells/SCCS/s.device.shell       ***)
(***                                                                       ***)
(***        jg     SccsId = (#)  device.shell  2.1 4/27/90 15:18:19        ***)
(*****************************************************************************)
(***                                                                       ***)
(***       Revision History:                                               ***)
(***        0.0     03/11/86    Implementatoin of revision tracking        ***)
(***        0.1     03/15/86    Correction of spelling errors              ***)
(***        1.0     08/01/86    Conversion for 4.0 software                ***)
(***        2.0     10/20/88    Conversion for CAGEN Software              ***)
(***        2.1     12/19/88    Cleanup of routines                        ***)
(***  08/03/98  jmt  A3  Add Z1F281FxxPJM, S4D27RFxxPJM, HS4D27RFxxIN to   ***)
(***                     TIDeviceType case stmt for pinmaps.               ***)
(***  02/18/99  jmt  A6  Moved AN6 from EVENPINS list to ODDPINS.          ***)
(***  06/29/99  jmt  A8  Made _RESET an IO in DeviceTables. Took _RESET out***)
(***                     of EVENINS & LEAK_lessPINS pinlists.              ***)
(*****************************************************************************)
...
const
    ...
    MAXV    = 8v;                  (* Maximum pin voltage allowed             *)
    MINI    = -50ma;               (* Minimum pin current allowed             *)
    MAXI    = 50ma;                (* Maximum pin current allowed             *)
    MINVCC  = -4v;                 (* Minimum VCC voltage allowed             *)
    MAXVCC  = 8v;                  (* Maximum VCC voltage allowed             *)
    MINVCCI = -200000;             (* Minimum VCC current allowed in ua       *)
    MAXVCCI = 400000;              (* Maximum VCC current allowed in ua       *)

(******************************************************************************)
(*  theses procedures are set here to avoid compile errors in other sites     *)
(*  than MIHO -  They have to be commented in MIHO                            *)
(******************************************************************************)

procedure TIJPinSetPP( var pl : PinList; pinnum, testerpin : integer;
		     pintype : PinType; minv, maxv : MicroVolts;
		     mini, maxi : NanoAmps ); 
begin
PinSetPP(pl,pinnum,testerpin,pintype,minv,maxv,mini,maxi);
end;
procedure TIJPinSetMuxPP( var pl : PinList; 
		      pinnum, testerpin1,testerpin2 : integer;
		      pintype : PinType; minv, maxv : MicroVolts;
		      mini, maxi : NanoAmps );
 
begin
PinSetMuxPP(pl,pinnum,testerpin1,testerpin2,pintype,minv,maxv,mini,maxi);
end;

(******************************************************************************)
(**********************end of procedures other than MIHO **********************)
(******************************************************************************)
...
case TIDeviceType of
   ...
   PinSetMP  (_RESET     ,  52 ,   5 ,     IO, MINV,   MAXV,   MINI,   MAXI );
...
{
   PinSetSupply(VDDTC16  ,  97 , Supply2 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VSSLC16  ,  64 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSTC16  ,  87 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSPC16  ,  14 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSAC16  ,  20 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSC8    ,  43 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSPC8   ,  39 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSAC8   ,  33 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSPLL   ,  62 , Ground ,           0  ,    0  ,    0   ,    0);
}
  end;

  HS4D27FxxIN,
  HS4D27RFxxIN :        (* SCAMP-RD64  Probe card description *)
     begin
       if miho_site then
         begin 
   ...
   TIJPinSetPP  (_RESET     ,  54,    71,    IO, MINV,   MAXV,   MINI,   MAXI );
...
{  Below are for info purpose

   PinSetSupply(VDDLC16  ,  73 , Supply1 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDPC16  ,  13 , Supply1 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDC8    ,  36 , Supply1 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDPC8   ,  38 , Supply1 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDAC8   ,  30 , Supply2 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDPLL   ,  63 , Supply2 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VDDTC16  ,  97 , Supply1 ,       MINVCC, MAXVCC, MINVCCI, MAXVCCI );
   PinSetSupply(VSSLC16  ,  64 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSTC16  ,  87 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSPC16  ,  14 , Ground ,           0  ,    0  ,    0   ,    0);
   PinSetSupply(VSSAC16  ,  20 , Ground ,           0  ,    0  ,    0   ,    0);
...
{  Below are for info purpose
  ...
  HS4D26CSFxxIN :      (* SCAMP-RD48  Probe card description *)
     begin
   ...
   TIJPinSetPP  (_RESET     ,  52 ,  71   ,      IO, MINV,   MAXV,   MINI,   MAXI );
...
{  Below are for info purpose
  ...
  DeviceTableClose;
     ...
     PinListSet     (Rst_Clk,_RESET,CLKOUT,OSCIN,OSCIN,OSCOUT);
     (*       These fields correspond to Pattern fields                       *)
...
(* define C8/c16 outs & ios excluding TAP ios *)
     PinListSet (ioc8,C8_BRL1,C8_BRL2);
     PinListSet (outc8,C8_REL{,C8_DPOUT,C8_SHFTCLK});
     PinListSet (ioc16,PORTB,PORTC,
                        SPI,SCITXD,SCICLK,
                        Interrupt,TCMP2,TCAP4,
                        TRIGOUTS,
                        C16_BRL1,C16_BRL2);
     PinListSet (outc16,         {,C8_DPOUT,C8_SHFTCLK}
                         TXCAN);
                        
(* define pin group as per app load group*)
PinListSet(bipull10,SPISTB,
                         TO4,TO3,TO2,TO1,
                         TCMP2,
                         C16_C_0,C16_C_1) ;  (* Bipull10 *)
PinListSet(gate,C16_B0,C16_B1,C16_B2,C16_B3); (* gate / gate1k *)
PinListSet(gate05,SPISIMO,SPICLK); (* gate05 *)
PinListSet(anad,C16_C_3,C16_C_4,C16_C_5,C16_C_6); (* anad *)
...
PinListSet(gpioc16,bipull10,gate,gate05,anad,opd20,opd10,pul250,pul500,bipol33);
    ...
    (*             Test Engineering defined Pin Lists                         *)
    (*                                                                        *)
     ...
     PinListSet     (ALLIOS,AN0,AN1,AN2,AN3,AN4,AN5,AN6,AN7,AN8,T11,T12,T21,T22,
                    ...
                    ,C8_BRL1,C8_REL,C8_SHFTCLK,C8_DPOUT,C8_TEST,_RESET,OSCIN
     ...
     PinListSet     (EVENPINS,AN0,AN2,AN4,AN8,T12,T22,RX2,T32,T42,C8_SI,C8_C0
                    ,C8_BRL2,C8_REL,C8_DPOUT,_RESET,OSCOUT,DP_CLKIN,C16_SHFTCLK
     ...
     PinListUnSet  (NOTRISTATEOUTS, EVENTRISTATE);
               (* remove eventristate pins                                    *)
     PinListUnSet  (NOTRISTATEOUTS, ODDTRISTATE);
               (* remove oddtristate pins                                     *)
               (* leaves notristate outputs                                   *)
