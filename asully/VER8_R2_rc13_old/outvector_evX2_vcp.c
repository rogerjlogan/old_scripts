...
/***************************************************************************************/
/***************************************************************************************/
int outputvector_evX2(FILE * fpout1, char *out_vector, char *wft, char *DMI,
				 long repeatcount, char *comment, char *microinstr, int CPM, char *pb_vector,int line) {
	char tempstr[MULTILINESIZE + 1], tempstr2[MULTILINESIZE + 1], commentstr[MULTILINESIZE + 1];
	char tempstr_evX2[MULTILINESIZE + 1], tempstr2_evX2[MULTILINESIZE + 1];
	char shortcomment[MULTILINESIZE + 1];
	static char vector_hold[READLINELEN + 1], wft_hold[MAXGCTLEN + 1];
	static char printhold[MULTILINESIZEBIG + 1], microhold[50]; 
	static char printhold_evX2[MULTILINESIZEBIG + 1];
...
//	fprintf(fpout1,"\"XCC%d\"\n",callcount);
	...
	if (flush_buff) {
		if (repeat_counter>1) {
			if (commentstr[0] == '\0') {
				if (printhold[0]!='\0') fprintf(fpout1, "%s <RPT %ld> \"FBCC0\"\n%s\"01CC%d\"\n", printhold_evX2, repeat_counter, printhold,callcount); 
				printhold[0] = '\0';
				printholdlen = 0;
				vector_hold[0] = '\0';
				repeat_counter = 0;
				...
				if (printhold[0]!='\0') fprintf(fpout1, "%s <RPT %ld>\"FBCC0a\"\n%s\"VNIR7=%ld %s 02CC%d\"\n",printhold_evX2, repeat_counter, printhold, VNIR, commentstr,callcount); 
				printhold[0] = '\0';
				printholdlen = 0;
				vector_hold[0] = '\0';
				commentstr[0] = '\0';
			...
			if (commentstr[0] == '\0') {
				if (printhold[0]!='\0') fprintf(fpout1, "%s\"FBCC0b\"\n%s \"03CC%d\"\n",printhold_evX2, printhold,callcount); 
				printhold[0] = '\0';
				printholdlen = 0;
				vector_hold[0] = '\0';
				repeat_counter = 0;
				...
				if (printhold[0]!='\0') fprintf(fpout1, "%s\"FBCC0c\"\n%s \"VNIR7=%ld %s 04CC%d\"\n",printhold_evX2, printhold, VNIR, commentstr,callcount); 
				printhold[0] = '\0';
				printholdlen = 0;
				vector_hold[0] = '\0';
				commentstr[0] = '\0';
		...
		if (!CPM) {
			if (repeat_counter > 1) {
				while (repeat_counter > max_repeat) {
					fprintf(fpout1, "%s <RPT %d>\"0x0xRC1\"\n%s \"05CC%d\"\n", printhold_evX2, max_repeat, printhold,callcount);	/* it is wrong to have "VNIR+=max_repeat;" here!!! */
					VC++;
...
//				fprintf(fpout1, "%s \"06CC%d\"\n", out_vector,callcount);	/* output the end label */
				...
				fprintf(fpout1, "%s <RPT %ld>\"0x0xRC2\"\n%s\"VNIR0=%ld 07CC%d\"\n", printhold_evX2, repeat_counter, printhold, VNIR,callcount);	/* it is wrong to have "VNIR+=max_repeat;" here!!! */
				VC++;
			}
			else {
				...
				if (commentstr[0] == '\0' || SUPPRESS_COMMENTS) {
					fprintf(fpout1, "%s\n%s\"VNIR1=%ld 09CC%d\"\n", printhold_evX2, printhold, VNIR,callcount);
					VC++;
...
/* If CPM then we need to spit out the last vector BEFORE the end label because we add the RET */
		if (CPM) {
			if (repeat_counter > 1) {
				while (repeat_counter > max_repeat) {
					fprintf(fpout1, "%s <RPT %d>\"0x0xRC3\"\n%s \"11CC%d\"\n", printhold_evX2, max_repeat, printhold,callcount);	/* it is wrong to have "VNIR+=max_repeat;" here!!! */
					VC++;
					repeat_counter -= max_repeat;
					VRE++;
				}
				fprintf(fpout1, "%s <RPT %ld>\"0x0xRC4\"\n%s \"12CC%d\"\n", printhold_evX2, repeat_counter, printhold,callcount);	/* it is wrong to have "VNIR+=max_repeat;" here!!! */
				VC++;
			}
			else {
				pare_comment(shortcomment, commentstr);
			...
			fprintf(fpout1, "%s \"15CC%d\"\n", out_vector,callcount);	/* output the end label */
				if (commentstr[0] == '\0' || SUPPRESS_COMMENTS) {
					...
					fprintf(fpout1, "%s\n%s \"%s 14CC%d\"\n", printhold_evX2, printhold, commentstr,callcount);
					VC++;
				}
			}
...
/* if shutdown available print it out to close any open strobes, C32 had this issue */
		if (found_shut) {
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_evX2_SHUTDOWN1\"\n",EVX2VECT,PUSHBACK_CYCLE,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_SHUTDOWN1\"\n",vector_shut2,REMAP_DEF_WFT,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_evX2_SHUTDOWN2\"\n",EVX2VECT,PUSHBACK_CYCLE,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_SHUTDOWN2\"\n",vector_shut2,REMAP_DEF_WFT,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_evX2_SHUTDOWN3\"\n",EVX2VECT,PUSHBACK_CYCLE,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_SHUTDOWN3\"\n",vector_shut2,REMAP_DEF_WFT,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_evX2_SHUTDOWN4\"\n",EVX2VECT,PUSHBACK_CYCLE,NORMALSGHDR);
			fprintf(fpout1, "* %s* %s %s; \"AUTO_VCP_SHUTDOWN4\"\n",vector_shut2,REMAP_DEF_WFT,NORMALSGHDR);
			}
	...
	if ((vector_is_new || vector_is_label || vector_is_microinstr) && printhold[0] != 0) {
		...
		if (repeat_counter > 1) {
			...
			strcpy(tempstr2_evX2, printhold_evX2);
			printhold[0] = '\0';
			printhold_evX2[0] = '\0';
			printholdlen = 0;
			while (repeat_counter > max_repeat) {
				...
				sprintf(tempstr, "%s\n", tempstr2);
				VC++;
				sprintf(tempstr_evX2, "%s <RPT %d>\"XYZCCrc5\"\n", tempstr2_evX2, max_repeat);
				...
				strncat(printhold_evX2, tempstr_evX2, MULTILINESIZEBIG);
				printholdlen += strlen(tempstr);
				
				/* Think we need to flush this out here... */

				fprintf(fpout1,"%s%s",printhold_evX2,printhold);
				printhold[0]='\0';
				printhold_evX2[0]='\0';
				printholdlen = 0;
			...
			sprintf(tempstr, "%s ", tempstr2);
			VC++;
			sprintf(tempstr_evX2, "%s <RPT %ld> ", tempstr2_evX2, repeat_counter);
			if (printholdlen + strlen(tempstr) > MULTILINESIZEBIG) {
				fprintf(fpout1, "%s\n%s 21CC%d\n", printhold_evX2, printhold,callcount);
				printhold[0] = '\0';
				printholdlen = 0;
		...
		if (commentstr[0] == '\0' || SUPPRESS_COMMENTS) {
			VC++;
			fprintf(fpout1, "%s\n%s \"22CC%d,%d\"\n", printhold_evX2, printhold,callcount,line);
		}						/* if there isn't a comment don't print one */
		else {
			VC++;
			fprintf(fpout1, "%s\n%s \"VNIR7=%ld %s 23CC%d,%d\"\n", printhold_evX2, printhold, VNIR, commentstr,callcount,line);
		}
		commentstr[0] = '\0';
		printhold[0] = '\0';
		printhold_evX2[0] = '\0';
	...
	if (vector_is_vector) {
		...
		if (vector_matches_prev && REPEAT_COMPRESSION) {
			...
			sprintf(printhold_evX2, "* %s* %s %s;", pb_vector, PUSHBACK_CYCLE, NORMALSGHDR);
		}
...
//		fprintf(fpout1,"\\%s\\\n",vector_hold);
		...
		sprintf(tempstr_evX2, "%s", out_vector);
		strcpy(printhold, tempstr);
		strcpy(printhold_evX2, tempstr_evX2);
		printholdlen = strlen(printhold);
		...
		if (DEBUG) fprintf(fpstatus, "&R_C:%ldRC:%ld,H:%s\n", repeat_counter, repeatcount, printhold);
		sprintf(tempstr, "* %s* %s %s; \"REMAP %s to %s\"", out_vector, REMAP_DEF_WFT, NORMALSGHDR,  DEF_WFT, REMAP_DEF_WFT);
		sprintf(tempstr_evX2, "* %s* %s %s; %s", pb_vector, PUSHBACK_CYCLE, NORMALSGHDR, microinstr);
		strcpy(printhold, tempstr);
		strcpy(printhold_evX2, tempstr_evX2);
		printholdlen = strlen(printhold);
		strcpy(vector_hold, out_vector);	/* keep for check repeat compression */
