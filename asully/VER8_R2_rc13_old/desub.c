...
/* file variables */
	...
	char command[MEMOSTRLEN], readline[READLINELEN + 1], commentbuf[READLINELEN + 1], tempstr[READLINELEN + 1];
	char pbuf[READLINELEN + 1];
	char *F1BUF, *F2BUF, *ptrSBR, *ptrSBR1, *ptrSBR2;
	char *ptrZ, *ptrCR, *ptrNCR, *ptrOP, *ptrCL, *ptrcommaSTART, *ptrcommaSTOP, *ptrVALFUNC, *ptrFND, *ptrEND;
...
#define MAXSBRT 20
	...
	if (NULL == (fpstatus = fopen(TRANSLATION_STATUS, "a"))) {	/* replaces the fpstatus messages */
		fprintf(stderr, "\nFATAL_ERROR:%s:Could not open %s for append output[%s,%d,%s]\n", G_compilestr, TRANSLATION_STATUS, __FILE__, __LINE__, __FUNCTION__);
		exit(1);
	}
	...
	F2BUF[bytes2read2] = '\n';
	for (i=1;i<500;i++) {
		F1BUF[bytes2read1 + 1] = 	F2BUF[bytes2read2 + 1] = '\0';
		}
	fclose(fpin1);
	fclose(fpin2);
	if (gzin1) {
...
/* step 2: save the name of the subroutine and its start */
		if (strstr(ptrCR, "external hardware subroutine") != NULL ||
		...
		else if ((ptrSBR1 = strstr(ptrCR, "subroutine")) != NULL
				 || (ptrSBR2 = strstr(ptrCR, "Subroutine")) != NULL) {
			maxsbrt++;
			if (ptrSBR1 != NULL) ptrSBR = ptrSBR1;
			if (ptrSBR2 != NULL) ptrSBR = ptrSBR2;
			subrt[maxsbrt].numlines = subrt[maxsbrt].containsmatch = 0;
			subrt[maxsbrt].name = ptrSBR + sizeof("subroutine");	/* this is a ptr */
			beginend = started = 0;
			...
			while (ptrOP <= ptrCL && ptrOP != NULL && ptrCL != NULL) {
				...
				if (!isspace(*ptrOP)) {
					subrt[maxsbrt].paramnames[subrt[maxsbrt].numparams][i++] = *(ptrOP++);
				}
				ptrCR = ptrOP;
			}
			while (isspace(*ptrZ)) *(ptrZ--) = '\0';	/* blank out any terminating spaces */
			fprintf(stdout, "SBRFND[%d][%s]\n", maxsbrt, subrt[maxsbrt].name);
			subrt[maxsbrt].paramnames[subrt[maxsbrt].numparams][i++] = ';';
			subrt[maxsbrt].paramnames[subrt[maxsbrt].numparams][i] = '\0';
			if (i <= 1) subrt[maxsbrt].numparams--;	/* so, if there are no parameters i=-1 */
		}
...
/* done with this subroutine */
			...
			fprintf(stdout, "         this will force CPM for all patterns that use it!\n");
			fprintf(fpstatus, "Warning, subroutine[%s] contains match!\n", subrt[maxsbrt].name);
			fprintf(fpstatus, "         this will force CPM for all patterns that use it!\n");
...
/****************** now run with pattern to translate *******************/
	ptrCR = F1BUF;
	while ((ptrNCR = strchr(ptrCR, '\n')) != NULL && ptrCR < (F1BUF + bytes2read1)) {
		*ptrNCR = '\0';
		commentbuf[0] = '\0';
		multiline =
			remove_comments(ptrCR, commentbuf, KEEP_YES, F1BUF + bytes2read1);
//              fprintf(stdout,"***[%s]<%s>%p,%p\n",ptrCR,commentbuf,ptrCR,ptrNCR);
		if (multiline) {
			ptrNCR = strchr(ptrCR, '\n');
			if (ptrNCR != NULL) *ptrNCR = '\0';
		}
		ptrNCR = find_close(ptrCR, ptrNCR, commentbuf, F1BUF + bytes2read1);
		if (ptrNCR==NULL) {
...
/* now we only print the subroutines that we actually use! */
			for (i = 0; i <= maxsbrt; i++) {
				if ((ptrFND=strstr(ptrCR, subrt[i].name)) != NULL) {
					ptrEND = ptrFND + strlen(subrt[i].name);
					if (!isspace(*ptrEND) && *ptrEND != '(') {
//						fprintf(stdout,"OOPS2, too long![%s][%s][%s]\n",ptrCR,ptrFND,subrt[i].name);
						continue;
						} 
					if (!isspace(*(ptrFND-1))) {
//						fprintf(stdout,"OOPS, not complete![%s],[%s],[%s]\n",ptrCR,ptrFND,subrt[i].name);
						continue;
						}
					sbrtcnt++;
					if (subrt[i].containsmatch == 1) {
						fprintf(stderr, "Pattern[%s] uses match subroutine[%s] (forces CPM)\n", infile1, subrt[i].name);
						fprintf(stdout, "Pattern[%s] uses match subroutine[%s] (forces CPM)\n", infile1, subrt[i].name);
						fprintf(fperrors, "Pattern[%s] uses match subroutine[%s] (forces CPM)\n", infile1, subrt[i].name);
						fprintf(fperrors, "   You may want to attempt this with threading instead.\n");
					}
					fprintf(stdout, "SBR[%s][%d]NL=%d\n", subrt[i].name, i, subrt[i].numlines);
					subrt[i].used = 1;
			...
			for (found = i = 0; i <= maxsbrt; i++) {
				...
				if ((ptrVALFUNC = strstr(readline, subrt[i].name)) != NULL) {
					...
					good = 0;
					fprintf(fpstatus, "Validating if this is[%s] a function call...", subrt[i].name);
					while (*ptrVALFUNC != '\n') {
					...
					if (good) {
						if (subrt[i].used == 0 || subrt[i].used == -1) {
							subrt[i].used = -1;
							fprintf(stderr, "Pattern[%s] uses subroutine[%s] but does not\n", infile1, subrt[i].name);
							fprintf(stderr, " reference it with \"external subroutine\"; not installing subroutine\n");
							fprintf(fperrors, "Pattern[%s] uses subroutine[%s] but does not\n", infile1, subrt[i].name);
							fprintf(fperrors, " reference it with \"external subroutine\"; not expanding this subroutine\n");
							continue;
						}
//                                              fprintf(stdout,"Next is subroutine [%s]\n",subrt[i].name);
						gzprintf(fpout, "{Next is subroutine [%s] comment[%s]}\nbegin\n", subrt[i].name, commentbuf);
						for (j = 0; j < subrt[i].numlines; j++) {
							if (subrt[i].numparams == -1) {
								...
								pprint = 0;
								ptrOP = strchr(ptrCR, '(');	/* ) */
/* ( */ ptrcommaSTOP = ptrCL = strstr(ptrCR, ");");
								if (ptrCL != NULL) *ptrCL = '\0';
								for (k = 0; k <= subrt[i].numparams; k++) {
									if (strstr(subrt[i].lines[j], subrt[i].paramnames[k]) != NULL) {	/* its a parameter */
										ptrcommaSTART = ptrOP + 1;
										for (m = 0; m <= k; m++) {
											ptrcommaSTOP = strchr(ptrcommaSTART, ']');
											if (ptrcommaSTOP != NULL) {
												*(ptrcommaSTOP + 1) = '\0';
												if (m != k) ptrcommaSTART = ptrcommaSTOP + 2;
											}
										}
										pprint++;
										found = 1;
										remove_multispace(ptrcommaSTART, pbuf);
										sprintf(tempstr, "%s;{%s}\n", pbuf, subrt[i].lines[j]);
										gzprintf(fpout, "%s", tempstr);
									}
								}	/* k */
								if (pprint == 0) {
									remove_multispace(subrt[i].lines[j], pbuf);
									gzprintf(fpout, "%s\n", pbuf);
								}
							}
						}		/* j */
	...
	for (i = 0; i <= maxsbrt; i++)
		if (subrt[i].used == 1) fprintf(stdout, ",%s", subrt[i].name);
	fputc('\n', stdout);
	...
	while ((ptrOP = strstr(ptrCR, "(*")) != NULL) {
		...
		if (ptrCL != NULL) {
			...
			*(ptrOP++) = ' ';
			...
			if (KEEP == KEEP_NO) ptrCCP = comment;	/* reset it if we are not keeping it */
			*(ptrOP++) = ' ';
			*(ptrOP++) = ' ';
			removedp++;
			ptrCR = ptrCL + 2;
//                   if (*ptrCR=='\0') break;
			if (ptrCR >= stophere) break;
		}
//           else {fprintf(stdout,"Couldn't find close(**) [%s]\n",ptrOP);exit(1);}
		else {
			len = strlen(ptrCR);
			ptrNEW = ptrCR + len;
			if (ptrNEW >= stophere) break;
			*ptrNEW = '\n';		/* put carriage returns back in if we hit end of line */
			ptrTEMP = strchr(ptrNEW + 1, '\n');
			if (ptrTEMP != NULL) *ptrTEMP = '\0';
			fprintf(fpstatus, "Opening up comment1 of len[%d] for more lines, len[%d]...\n", len, strlen(ptrCR));
			multiline = 1;
		}
	}
	ptrCR = inout;
...
//                    if (*ptrCR=='\0') break;
			...
			ptrNEW = ptrCR + len;
			if (ptrNEW >= stophere) break;
			fprintf(fpstatus, "Opening up comment2 of len[%d] for more lines, len[%d]...\n", len, strlen(ptrCR));
			*ptrNEW = '\n';		/* put carriage returns back in if we hit end of line */
			ptrTEMP = strchr(ptrNEW + 1, '\n');
			if (ptrTEMP != NULL) *ptrTEMP = '\0';
			multiline = 1;
...
//    fprintf(stdout,"PTST[%p],ptrCR[%p],stop[%p],IN[%d]\n",ptrST,ptrCR,stophere,openparen);
	...
	if (openparen != 0) {
		*ptrCR = ' ';
		...
		linecnt++;
		if (ptrNEWCR == NULL) {
			while ((*ptrCR==' ' || *ptrCR== '\t') && ptrCR < stophere) ptrCR++;
			if (ptrCR != stophere && ptrCR!=(stophere-1)) {
