...
def modifyInherits(wftarray):
    """
        Modify inherits to include cell names necessary for each category
    """
    ...
    for WFT in newWFTs:
        ...
        #go thru each inherit of given wft
        for inhrt in inhrts:
            #index wft from inherit and go thru each cell
            for cell in newWFTs[inhrt]['cells']:
                cat=cat_patrn.search(cell['cellname'])
                for pc in __psblcats__:
                    if filter(lambda x:x in list(pc),list(cat.group('cat'))):
                        if WFT+'_'+pc not in finalWFTs.keys():
                            finalWFTs[WFT+'_'+pc]={'per':per,'com':com,'inhrts':[],'cells':cells}
                        if inhrt+'.'+cell['cellname'] not in finalWFTs[WFT+'_'+pc]['inhrts']:
                            finalWFTs[WFT+'_'+pc]['inhrts'].append(inhrt+'.'+cell['cellname'])
                #End for
            #End for
        #End for
...
#                    UNION=set.union(set(pinlist), set(npl))
    ...
    mod_wftarray=modifyInherits(wftarray)
