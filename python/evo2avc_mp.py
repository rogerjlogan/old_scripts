...
def getPins(sighdr_content):
    global _evo_sh_,_avc_sh_,_extra_pins_,_missing_pins_,_out_sh_,_xp_found_,_xp_msg_,_mp_found_,_mp_msg_,_ports_
    ...
    if not prtObj:
        print "Warning !!! No ports found.  Creating only one avc with arbitrary name pALL!"
        _ports_ = {'pALL':','.join(_out_sh_)}
    else:
...
def main(fileN,port,sighdr,inds=None):
    global _v_cnt_,_c_cnt_
    newfileN = fileN.replace('.evo','@'+port+'.avc')
    ...
    _v_cnt_=0
    _c_cnt_=0
    for _v_cnt_,line in enumerate(myOpen(fileN)):
        repeat=1
