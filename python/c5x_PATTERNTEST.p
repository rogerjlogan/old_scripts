...
if (not (actablename in [TIlofrqACTable,TIlofrqACTable1,TIlofrqlooseACTable])) and RamSequenceTestEnable then
   begin
...
if (not (actablename in [TIlofrqACTable,TIlofrqACTable1,TIlofrqlooseACTable])) and RamWalkTestEnable then
   begin
...
if RomTestEnable then begin
if ( (not myTestFunc(rom_prot_tc)) and RomProtectEnable ) then
   begin
     c5x_PATTERNTEST:=false;
     failrom:=true;
...
if ( myTestFunc(rom_prot_tc) and (not RomProtectEnable) ) then
   begin
     c5x_PATTERNTEST:=false;
     failrom:=true;
...
if ( (not myTestFunc(bootcode_tc)) and BootCodeEnable ) then
   begin
     c5x_PATTERNTEST:=false;
     failrom:=true;
...
if SecondPassEnable and (not SingleSpecActable) and
