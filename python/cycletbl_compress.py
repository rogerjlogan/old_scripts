#--------------------------------------------------------------------------
#     THIS FILE COMPRESSES cycletbl.p FROM POLARIS AND CREATES A ZIPPER
#     
#     IN ORDER TO EXCLUDE CERTAIN GROUPS ADD THEM TO THE device.p IN THE FOLLOWING FUNCTION
#     
#     EXAMPLE:
#     EXCLUDE_THESE_GROUPS(FIELD1,FIELD2,FIELD3)
#     
#     BE SURE TO SEPARATE BY COMMAS
# 
# 
# 
#--------------------------------------------------------------------------
...
__AUTHOR__ = "AUTHOR: Roger Logan, Anora"
__DATEMODIFIED__ = "LAST MODIFIED: May 10, 2012"
__VERSION__ = "v1.2"
__FILENAME__ = "cycletbl_compress.py"
__INPUT1_FILENAME__ = "<cycletbl.p>"
__INPUT2_FILENAME__ = "<device.p>"
__OUTPUT1_FILENAME__ = "<cycletbl_mod.p>"
__OUTPUT2_FILENAME__ = "pinlist.txt"
__OUTPUT3_FILENAME__ = "zipper.evo"
__OUTPUT4_FILENAME__ = "analysis.txt"
...
def formatCycSets(envCycTbls):
    ...
    for t in envCycTbls:
        for cell in envCycTbls[t]['pins']:
            d0d1=cell.split(',')
            d0,d1=d0d1[0],d0d1[1]
            pins=envCycTbls[t]['pins'][cell].split(',')
            for pin in pins:
                row = {'cycletbl':'WFT_'+str(t)+',','d0':d0+',','d1': d1+',','pins':pin}
...
#            __apexpins__.append(pin)
    ...
    for vmaptpl in vmapset:
        ...
        for pin in getPins(vmaptpl[2]):
            ...
            count+=1
            if count==1:
                vmap_str+='\n\tVectorMapSet('+vmaptpl[0]+','+vmaptpl[1]+','+vmaptpl[2]+','+vmaptpl[3]+')'
#        vmap     = vmaptpl[0]
#        vfield   = vmaptpl[1]
#        vpinlist = vmaptpl[2]
#        vbase    = vmaptpl[3]
...
#
#################
    zstr,astr=formatZippers(envCycTbls)
    ...
    if __DEBUG__:
        printstr += "\n"+__OUTPUT4_FILENAME__
    print printstr
    out1file.write(astr)
    out1file.write(ostr)
    out1file.write(vmap_str)
    out1file.close()
