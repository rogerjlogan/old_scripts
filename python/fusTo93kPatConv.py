...
def main(fileN, inds=None):
    ...
    outFile.write(buildPinlistHdr(__new_sh__))
        ...
        if vecObj:
            ...
            for pin in __new_sh__:
                if pin in orig_vect_dict.keys():
                    new_vect_dict[pin] = orig_vect_dict[pin]
                else:
                    new_vect_dict[pin] = __fill_char__
            vectLst = []
            for v in __new_sh__:
                vectLst.append(new_vect_dict[v])
            
            new_vect = ''.join(vectLst)+'\n'
