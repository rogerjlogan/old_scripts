   ...
   while (strstr(vector,"(*")!=NULL) {
      ...
      if ((ptrOPEN=strstr(vector,"(*"))!=NULL) { /* comments */
         ...
         if (ptrCLOSE!=NULL) {
            ...
            if (*ptrCLOSE=='*') fprintf(fperrors,"could be unterminated comment in [%s]:[%s]\n",infile_orig,vector);
   ...
   *begin_loc=-1;
   if ((BP=strstr(vector,"begin"))!=NULL){
      ...
      *(BP+4)=' ';/* n */
      ...
      if ((*loopnest)>4) {
         fprintf(fperrors,"FATAL:Loop nesting too deep[%d] in [%s]!,exiting\n",*loopnest,infile_orig);
         fprintf(stderr,"FATAL:Loop nesting too deep[%d] in [%s]!,exiting\n",*loopnest,infile_orig);
         exit(1);
         }
      }
   return(0);
...
int check_for_repeat(char *vector,int *repeat_loc,int *repeatloop,int loopnest){
   ...
   *repeat_loc=-1;
   if ((RP=strstr(vector,"repeat"))!=NULL) {
      ...
      *(RP+4)=' ';/* a */
      *(RP+5)=' ';/* t */
      if (*(RP+6)!=' ' || *(RP+6)!='\t') *repeatloop=get_count(RP+6);
      else *repeatloop=get_count(RP+7);
      if (*repeatloop==RET_FOREVER) return(RET_FOREVER);
      }
   else *repeatloop=0;
   return(0);
   }
...
int check_for_match(char *vector,int *match_loc,int *matchloop){
   char *MP;
   *match_loc=NO_MATCH;
   if ((MP=strstr(vector,"match"))!=NULL){
...
int check_for_shutdown(char *vector,int *shutdown_loc){
   char *SD,*SD1;
   *shutdown_loc=-1;
   if ((SD=strstr(vector,"shutdown"))!=NULL||(SD1=strstr(vector,"Shutdown"))!=NULL){
      if (SD==NULL && SD1!=NULL) SD=SD1;
...
int check_for_align(char *vector){
   ...
   if ((AP=strstr(vector,"align"))!=NULL){
      fprintf(fperrors,"FYI_ALIGN_FOUND:found align Polaris instruction in %s! skipping\n",infile_orig);
      fprintf(fpstatus,"FYI_ALIGN_FOUND:found align Polaris instruction in %s! skipping\n",infile_orig);    
      align_loc=AP-vector;
      *(AP+0)=' ';/* a */
      *(AP+1)=' ';/* l */
      *(AP+2)=' ';/* i */
...
int check_for_subroutine(char *vector){
   ...
   if ((AP=strstr(vector,"subroutine"))!=NULL){
      fprintf(fperrors,"FYI_SUBROUTINE_FOUND:found Polaris subroutine in [%s]! You must deal with this yourself\n",infile_orig);
      fprintf(fpstatus,"FYI_SUBROUTINE_FOUND:found Polaris subroutine in [%s]! You must deal with this yourself\n",infile_orig);    
...
int check_for_vector(char *vector,char *result_vector,int *vs,int *ve,char *label){
   ...
   *vs=*ve=0;
   ...
   if ((LBL=strchr(vector,':'))!=NULL) { /* found a label! */
      ...
      *LBL='\0';
      strcpy(label,vector);
      for (ptr=vector;ptr<=LBL;ptr++) *ptr=' ';
      }
   *vs=OB1-vector;
   *ve=CB1-vector;
   hold=*(SC+1);
...
int check_for_SET(char *vector,char *result_vector,int *vs,int *ve){
   ...
   int SET_FOUND=0;
   *vs=*ve=0;
   if ((SP=strstr(vector,"MSet"))!=NULL){SET_FOUND=1;}
   else if ((SP=strstr(vector,"DSet"))!=NULL){SET_FOUND=2;}
   else if ((SP=strstr(vector,"ISet"))!=NULL){SET_FOUND=3;}
   if ((SP=strstr(vector,"mset"))!=NULL){SET_FOUND=1;*(SP+0)='M';*(SP+1)='S';}/* capitalize it */
   else if ((SP=strstr(vector,"dset"))!=NULL){SET_FOUND=2;*(SP+0)='D';*(SP+1)='S';}/* capitalize it */
   else if ((SP=strstr(vector,"iset"))!=NULL){SET_FOUND=3;*(SP+0)='I';*(SP+1)='S';}/* capitalize it */
...
int get_count(char *ptr){
   ...
   else retval=(int)strtol(ptr,NULL,10);
   if (retval==-2) {
...
char * get_count_orig(char *ptr,int offset,long *count,char *vector){
   ...
   *count=0;
   ...
   retptr=ptr+offset;
   pos=ptr-vector;
   if(debug&4) fprintf(fpstatus,"WE ACTUALLY MADE IT HERE %s\n",vector);
   for (i=0;i<pos;i++) if (vector[i]!=' ' && vector[i]!='\t') {
      fprintf(fpstatus,"NON_FATAL_ERROR:We can't deal with this repeat!![%s]%d %d\n",vector,i,offset);
      fprintf(fperrors,"NON_FATAL_ERROR:%s:We can't deal with this repeat!![%s]%d %d\n",progstr,vector,i,offset);
   ...
   if ((ptr2=strstr(vector,"(*"))!=NULL && strstr(vector,"repeat")!=0) { /* blank out any comments before looking at this vector */
      ...
      if (vector[i]=='*' && vector[i+1]==')') {vector[i]=' ';vector[i+1]=' ';} /* finish off the tail */
      fprintf(fpstatus,"[%s]\n",vector);
      }
   ...
   if(debug&4) fprintf(fpstatus,"WE ACTUALLY MADE IT HERE\n");
   if (*count==0){ /* check for 'forever' */
      if (0!=strstr(ptr,"forever")) *count=RET_FOREVER;
      }
   return(retptr);
   }
