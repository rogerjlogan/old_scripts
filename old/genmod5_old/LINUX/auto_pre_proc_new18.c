...
char infile_local[READLINELEN+1];
int i,j,incomment;
...
fprintf(fpstatus,"\n   FT:FILE[%s]\n",infile_orig);
fprintf(fpstatus,"\n   FT:"); for(i=0;i<fields;i++) fprintf(fpstatus,"__%c,",fieldtype[i]);
fprintf(fpstatus,"\n   FC:"); for(i=0;i<fields;i++) fprintf(fpstatus,"%03d,",fieldcount[i]);
fprintf(fpstatus,"\n   FS:"); for(i=0;i<fields;i++) fprintf(fpstatus,"%03d,",fieldstart[i]);
fprintf(fpstatus,"\nPHDST:"); for(i=0;i<fields;i++) fprintf(fpstatus,"%03d,",phd_st[i]);
fprintf(fpstatus,"\nPHDSP:"); for(i=0;i<fields;i++) fprintf(fpstatus,"%03d,",phd_sp[i]);
fprintf(fpstatus,"\n CMMA:"); for(i=0;i<fields;i++) fprintf(fpstatus,"%03d,",cmma[i]);
fprintf(fpstatus,"\ncount:");
for(i=0;i<strlen(hexproc);i++) fputc('0'+i-10*(int)(i/10),fpstatus);fputc('\n',fpstatus);
fprintf(fpstatus,  "HXPRC:"); fprintf(fpstatus,"%s\n",hexproc);
fprintf(fpstatus,  "HXFLD:"); fprintf(fpstatus,"%s\n",hexfield);
fprintf(fpstatus,  "HX_ST:"); fprintf(fpstatus,"%s\n",hexdebug1);
fprintf(fpstatus,  "ST_SP:"); fprintf(fpstatus,"%s\n",hexdebug2);
fprintf(fpstatus,  "SIMFD:"); fprintf(fpstatus,"%s\n",hexdebug3);
...
ptr2=strrchr(infile_local,'/'); /* get the last '/' */
if (ptr2!=NULL) {
   strcpy(infile_local,ptr2);
   }

ptr=strrchr(infile_local,'.'); /* get the last '.' */
if (ptr!=NULL) {
   if (*(ptr+1)=='g' && *(ptr+2)=='z' && *(ptr+3)=='\0') gunzip=1;
   if (*(ptr+1)=='Z' && *(ptr+2)=='\0') gunzip=1;
   if (gunzip==1) {
      *ptr='\0';
      ptr3=strrchr(infile_local,'.'); /* get the last '.' i.e. .vpl */
      if (ptr3!=NULL) *ptr3='\0';
      sprintf(tempgunzipname,"%s.tmpgunzip",infile_local);
      sprintf(pattern,infile_local);
...
fprintf(stdout,"PRE_PROC_DPM/CPM:input file1 <%s> ok\n",(gunzip==1)?infile_local:infile_orig);
...
/*---*/
   ...
   if (CFRF==RET_FOREVER) flag_repeatforever=1;
   if (SUPER[SUPER_CNT].repeatcnt>0) {
      last_was_repeat=1;
      last_repeat[loopnest+1]=SUPER[SUPER_CNT].repeatcnt;
      }
/*---*/
   check_for_match(linebuf,&match_loc,&SUPER[SUPER_CNT].matchcnt);
   if (match_loc!=NO_MATCH) {flag_match=1;first_match_line=SUPER_CNT;last_was_repeat=0;}
/*---*/
   check_for_begin(linebuf,&begin_loc,&loopnest);
   if (begin_loc!=-1) {
      if (inshutdown>0) inshutdown++;
      begin_pending_index[loopnest]=SUPER_CNT;
      SUPER[SUPER_CNT].begin=1;
      if (last_was_repeat==1) {
         repeat_calc *= last_repeat[loopnest];
...
/*---*/
   ...
   if (end_loc!=-1) {
      ...
      if (last_was_repeat) {
         if (last_repeat[loopnest]!=0) repeat_calc /= last_repeat[loopnest];
         }
      loopnest--;
      if (loopnest<0) {
...
/*---*/
   ...
   CFV=0;
   if (CFS!=0) SUPER[SUPER_CNT].VectorType=VT_SET;
/*---*/
   else {/* can't be vector if already SET */
      CFV=check_for_vector(linebuf,SUPER[SUPER_CNT].vector,&vs,&ve,SUPER[SUPER_CNT].label);
      }
   if (CFV) {
      SUPER[SUPER_CNT].vectcount=vc++;
      SUPER[SUPER_CNT].VectorType=VT_Vector;
      if (inshutdown==1) inshutdown=0;/* anything great means its in a begin block which gets fixed elsewhere */
      SUPER[SUPER_CNT].shutdownvect=inshutdown;
      }
...
if (!flag_scan) {
   fprintf(fperrors,"FYI:%s:AUTO LAUNCHING vector_convert_polaris on pattern[%s]\n",progstr,outfile1);
